...
const RegisterProfile: NextPage<RegisterProfileProps> = () => {
	...
	useEffect(() => {
		if (user) {
			getUserData(user.get("ethAddress")).then(() => router.push("/profile"));
		}
	}, [router, user]);
	...
	const onSubmit: SubmitHandler<UserData> = (data) => {
		registerUser(data.username, data.email, data.password, data.walletAddress)
			.then((res) => {
				setShowModal(true);
			})
			.catch((error) => {
	...
	return (
		<>
			...
			<main className="min-h-screen flex flex-col  justify-center max-w-screen-xl mx-auto">
				<div className="bg-card flex flex-col md:flex-row items-center p-10 my-10 mx-10 gap-5">
					<div className="w-1/2 flex items-center justify-center  ">
						<img src="/assets/images/zatlogo.svg" alt="Zatcoin Logo" height={250} width={250} />
					</div>
					<div className="md:w-1/2 w-full  md:mt-0 mt-5 ">
						<h3 className="text-3xl font-bold text-zatcoin-blue-light uppercase tracking-wide mb-2">Register</h3>
						<span>Create your account at Zatcoin</span>
						<form onSubmit={handleSubmit(onSubmit)} className="space-y-4 my-10 md:w-4/5 w-full mx-auto md:mx-0 flex flex-col">
							...
							<div className="flex flex-col space-y-2">
								...
								{errors.email && <span className="text-red-500 text-sm">E-mail is required.</span>}
								<input
									type="email"
							...
							<div className="space-x-4 self-center pt-5">
								<button onClick={() => router.push("/profile")} type="button" className="btn-connect bg-red-500 hover:bg-red-600 shadow-red-500/50">
									Cancel
								</button>
								<button type="submit" className="btn-connect " disabled={isUserUpdating && true}>
									{isUserUpdating ? "Registering..." : "Register"}
								</button>
							</div>
						</form>
					</div>
			...
			<RegisterSuccess showModal={showModal} setShowModal={setShowModal} />
		</>
	);
};
