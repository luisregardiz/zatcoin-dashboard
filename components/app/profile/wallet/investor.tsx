...
const WalletInvestor: FC<WalletInvestorProps> = ({ id, title, description }) => {
	...
	return (
		<>
			<div>
				...
				<div className="md:p-20 p-10 bg-card flex flex-col md:flex-row items-center justify-center md:space-x-14 space-x-0 relative">
					<div className="md:w-auto  w-1/2">
						<img src={getInversorIcon(id)} alt="Inversor Image" height={180} width={180} />
					</div>
					<div className="flex flex-col md:w-1/2 w-auto md:mt-0 mt-5 ">
						<h3 className="font-bold text-xl mb-5 md:text-left text-center">You are a {title}</h3>
						<span className="font-bold text-gray-200 md:text-left text-center">Description:</span>
						<p className="mt-1 md:text-left text-center">{description}</p>
						<button
							onClick={() => logout()}
			...
			<InvestorTypeInfo showModal={showModal} setShowModal={setShowModal} />
		</>
	);
};
